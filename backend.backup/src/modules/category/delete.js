...
app.delete('/category/:id', [db_connection, admin_validator, connect_mongo], async(req, res)=>{

  const mongo = req.mongoDB;
  
  // searching for categoryss-turisticos collection in DB. If it does not exist it
  ...
  try{
    ...
    const response_db = await category_collection.deleteOne(query);

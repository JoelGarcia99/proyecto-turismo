...
export const startFetchingLocations = (from=0, limit=10, page=0)=>{

  //TODO: fix pagination
  return async(dispatch)=>{

    const res = await fetch(`${process.env.REACT_APP_API_HOST}/punto-turistico/?from=${from}&limit=${limit}`, {
      method: 'GET',
      headers: {
	'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8',
      }
    ...
    if(res.status !== 200) {
      ...
      dispatch( setLocationsData(jsonRes.puntos, jsonRes.range, page) );
    }
  }
}
